...
const Footer = () => (
  <section>
              ...
              <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                ...
                <ul className="list-none mt-4">
                {
                  footerLink.links.map((link, idx)=>(
                    ...
                    cursor-pointer ${ idx !== footerLink.links.length - 1 ? "mb-4" : "mb-0" }`}>
                        {link.name}
                    </li>
                  ))
                }
    ...
    md:flex-row flex-col pt-3 pb-3 border-t-[1px] border-t-[#3F3E45]">
      <p className="font-poppins 
      font-normal text-center text-[18px] 
      leading-[27px] text-white">
        2023 HooBank. All Righs Reserved.
      </p>
      <div className="flex flex-row md:mt-0 mt-6">
          {
            socialMedia.map((social, idx)=>(
              <img src={social.icon} alt="socialMedia" 
              key={social.id} className={`w-[21px] h-[21px] 
